...
def make_knot_squarefoil(nv=601, k_xy=0.05, k_z=0.05, open=True):
    ...
    for i in range(nv):
        theta = i * 2*PI / (nv-1)
        if theta >= PI/6.0 and star:
            str_idx = i
            star = False
        if theta < PI/6.0 or theta > PI/3.0:
    ...
    vertices[-1] += TOL * (vertices[-2] - vertices[-1])
    if not open:
        segs.append(np.array([len(vertices)-1, 0]))
...
if __name__ == "__main__":
    ...
    cylin_center = np.array([-0.11, 0.06, 0.0])
    ...
    model_name =  'grannyfoil' # 'trefoil', 'cinquefoil', 'sepfoil', 'fig8foil', 'squarefoil', 'grannyfoil'
    ...
    knot_length = 2. * PI * scaled_cylin_r * scaled_cylin_r
    tail_length = 0.5 * (init_config.length - knot_length )
    ...
    sim.add_shell_with_scale_3D(base_dir+"/input/cylinder_v363.obj", Vector3d(cylin_move[0], cylin_move[1], cylin_move[2]), Vector3d(cylin_scale[0], cylin_scale[1], cylin_scale[2]), \
        Vector3d(0.0, 0.0, 0), Vector3d(0, 1, 0), 0)
    # v, rotCenter, rotAxis, angVelDeg
    sim.set_DBC(Vector3d(-0.1, -0.1, -0.1), Vector3d(1.1, 1.1, 1.1), 
        Vector3d(-cylin_move[0]/t_total, -(cylin_move[1]-scaled_cylin_r)/t_total, 0), Vector3d(0, 0, 0), Vector3d(0, 0, 0), 0)

    sim.add_rod_3D(init_config.cipc_input, Vector3d(0.0, 0.0, 0),
            Vector3d(0, 0, 0), Vector3d(1, 0, 0), 0, Vector3d(1, 1, 1))
    # end vertex
    sim.set_DBC(Vector3d(0.852, 0.99, -0.1), Vector3d(1.1, 1.1, 0.55), 
        Vector3d(0.0, -0.1, 1.5*tail_length/t_total), Vector3d(0, 0, 0), Vector3d(1, 0, 0), 0)
    # start vertex
    sim.set_DBC(Vector3d(0.95, 0.668, -0.1), Vector3d(1.1, 0.75, 0.5), 
        Vector3d(-0.0, -0.05, -1.5*tail_length/t_total), Vector3d(0, 0, 0), Vector3d(1, 0, 0), 0)

    sim.MDBC_tmin = 0.0
    sim.MDBC_tmax = 0.5*t_total
    # # sim.MDBC_period = 1.0
