...
if __name__ == "__main__":
    ...
    fig8.cipc_input = base_dir + '/fig8foil.obj'
    fig8.write_cipc_input()
    fig8.write_curve(base_dir+'/', 'fig8foil_paraview.obj')
    fig8.sweep_curve()
    fig8.write_surface(base_dir+'/', 'fig8foil3D.obj')
    fig8.calcul_length()
    print("fig8foil length : %f" % fig8.length)

    # sim = Drivers.FEMDiscreteShellBase("double", 3)
    # sim.output_folder = base_dir+"/output/fig8foil/" #  + os.path.splitext(os.path.basename(sys.argv[0]))[0] + "/"
    ...
    # # cinque_knot = FlexibleRod()
    # # cinque_knot.r = r
    # # cinque_knot.read_from_file(base_dir + '/output/cinquefoil/', 'rod20.obj') 
    # # # helix.write_cipc_input()
    # # trans = -0.5 * (cinque_knot.v[0] + cinque_knot.v[-1])
    # # end_axis = cinque_knot.v[1] - cinque_knot.v[0]
    # # mid_axis = cinque_knot.v[int(cinque_knot.nvert/2)] + trans
    ...
    # # rot_axis /= rot_angle
    # # print(rot_axis)
    # # cinque_knot.trans_rot(trans, rot_axis, rot_angle)
    ...
    # # cinque_knot.write_curve(base_dir+'/', 'cinqueknot_paraview.obj')
    # # cinque_knot.sweep_curve()
    # # cinque_knot.write_surface(base_dir+'/', 'cinqueknot3D.obj')
    # # helix.calcul_length()
