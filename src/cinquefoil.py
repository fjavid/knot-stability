...
if __name__ == "__main__":
    ...
    # helix.cipc_input = base_dir + '/cinquefoil.obj'
    # helix.write_cipc_input()
    # helix.write_curve(base_dir+'/', 'cinquefoil_paraview.obj')
    # helix.sweep_curve()
    # helix.write_surface(base_dir+'/', 'cinquefoil3D.obj')
    # helix.calcul_length()
    # print("helix length : %f" % helix.length)

    # sim = Drivers.FEMDiscreteShellBase("double", 3)
    # sim.output_folder = base_dir+"/output/cinquefoil/" #  + os.path.splitext(os.path.basename(sys.argv[0]))[0] + "/"
