...
# file_dir = "/home/project/model1/input/"
# file_name = "rod2"
def obj_to_seg(file_dir, file_name):
    fullname = file_dir + file_name + ".obj"
    with open(fullname, 'r') as f:
        lines = f.readlines()
    for idx, line in enumerate(lines):
        sline = line.split(' ')
        if sline[0] == 'l' or sline[0] == 'f':
            nline = 's ' + sline[1] + ' ' + sline[2] + '\n'
            lines[idx] = nline
    cipc_fullname = file_dir + file_name + "_seg.obj"
    with open(cipc_fullname, 'w') as f:
        for line in lines:
            # print(line)
            f.write(line)

# # seg_to_curve_Obj("/home/project/model1/output/knot1/", "rod1")
# file_dir = "/home/project/model1/output/knot1/"
# file_name = "rod1"
# file_dir = "/home/project/inputs/"
# file_name = "f1x1"
...
def sweep_curve(file_dir, file_name, radius):
    ...
    # fullname = '/home/project/model1/output/knot1/rod3d.obj'
...
if __name__ == "__main__":
    ...
    sim.output_folder = "output/" + os.path.splitext(os.path.basename(sys.argv[0]))[0] + "/"
    # print(dir(Drivers))
    ...
    sim.add_shell_with_scale_3D("input/cylinder3K.obj", Vector3d(0, 0.0, 0), Vector3d(0.1, 1.0, 0.1), \
    ...
    knot1 = sim.add_rod_3D("input/rod2_seg.obj", Vector3d(0.0, -0.0, 0),
            Vector3d(0, 0, 0), Vector3d(1, 0, 0), 0, Vector3d(1, 1, 1))
    TOL = 0.00001
    # sim.set_DBC(Vector3d(-0.1, -0.1, -0.1), Vector3d(0.01, 1.01, 1.1), 
    #     Vector3d(-0.35, 0.25, 0), Vector3d(0, 0, 0), Vector3d(1, 0, 0), 0)
    sim.set_DBC(Vector3d(0.99, -0.1, -0.1), Vector3d(1.1, 1.01, 1.1), 
        Vector3d(0.35, 0.0, 0), Vector3d(0, 0, 0), Vector3d(1, 0, 0), 0)
    ...
    stiff_mult = 0.0001
    h = 5.0e-4
    sim.initialize_rod(rho, E, stiff_mult, thickness)
    
    sim.initialize_OIPC(thickness, 1.5e-4)
